...
class _HomePageState extends State<HomePage>
  ...
  Widget _buildAnimatedIOSWidget(
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        if (_carouselController.hasClients &&
          ...
          currentPage = _carouselController.page ??
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      extendBody: true,
      backgroundColor: _iosBgColor,
      // UPDATE 1: Menggunakan AnimatedSwitcher untuk transisi halus antar halaman
  ...
  // UPDATE 2: Menambahkan animasi Bounce pada ikon
  Widget _buildGlassIcon(int index, IconData icon, String label) {
    final isSelected = _selectedIndex == index;
    return GestureDetector(
      onTap: () {
        Feedback.forTap(context); // Getaran haptic
        _onItemTapped(index);
      },
      behavior: HitTestBehavior.opaque,
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 300),
        curve: Curves.easeOutBack, // Curve elastis untuk container
        padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 20),
        decoration: BoxDecoration(
          color: isSelected
        ...
        child: Column(
          ...
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            // Widget TweenAnimationBuilder untuk efek 'Pop' pada Icon
            TweenAnimationBuilder(
              duration: const Duration(milliseconds: 300),
              curve: Curves.elasticOut, // Efek mental (bouncy)
              tween: Tween<double>(
                  begin: 1.0, end: isSelected ? 1.0 : 0.8),
              // Trik: Saat selected, dia akan animate dari state sebelumnya ke 1.0 dengan elastic curve
              key: ValueKey(isSelected), // Kunci untuk memicu animasi ulang
            ...
            // Teks label dengan animasi slide/fade sederhana
            AnimatedSize(
              duration: const Duration(milliseconds: 200),
              curve: Curves.easeInOut,
              child: SizedBox(
                height: isSelected ? null : 0, // Hilangkan tinggi jika tidak selected
                child: isSelected
                    ? Padding(
